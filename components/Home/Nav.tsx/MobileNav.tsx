      ...
      <div className="text-white fixed justify-center flex flex-col h-full transform transition duration-500 delay-300 w- [80%] sm:w- [60%] bg-cyan-950 space-y-6 z- [100050] right-0">
        {NavLinks.map((link) => (
          <Link 
            ...
            href={link.url} 
          >
            <p  className="text-white w-fit text-x1 ml-12 border-b-[1.5px] pb-1 border-white sm:text-[30px]"> {link.label}</p>
            
          </Link>
        ))}
      <CgClose className="absoute top-[0.7rem] right-[1.4rem] sm:w-8 sm:h-8 w-6 h-6"/>
